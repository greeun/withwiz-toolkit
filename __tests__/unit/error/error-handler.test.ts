...
describe('Error Handler exports', () => {
  it('should export error handler functions', async () => {
    const errorHandler = await import('@withwiz/toolkit/next/error/error-handler');
...
describe('processError: AuthError mapping', () => {
  it('should map JWTError TOKEN_EXPIRED to 40103', async () => {
    const { processError } = await import('@withwiz/toolkit/next/error/error-handler');
    const { JWTError } = await import('@withwiz/toolkit/core/auth/errors');
  ...
  it('should map OAuthError to EXTERNAL_SERVICE_ERROR', async () => {
    const { processError } = await import('@withwiz/toolkit/next/error/error-handler');
    const { OAuthError } = await import('@withwiz/toolkit/core/auth/errors');
  ...
  it('should map PASSWORD_HASH_FAILED to SERVER_ERROR (not 400)', async () => {
    const { processError } = await import('@withwiz/toolkit/next/error/error-handler');
    const { PasswordError } = await import('@withwiz/toolkit/core/auth/errors');
...
describe('AUTH_ERROR_CODE_MAP completeness', () => {
  it('should contain all AUTH_ERROR_CODES keys', async () => {
    const { AUTH_ERROR_CODE_MAP } = await import('@withwiz/toolkit/next/error/error-handler');
    const { AUTH_ERROR_CODES } = await import('@withwiz/toolkit/core/auth/errors');
...
describe('ErrorResponse utility completeness', () => {
  it('should have all expected response factory methods', async () => {
    const { ErrorResponse } = await import('@withwiz/toolkit/next/error/error-handler');
...
describe('errorToResponse', () => {
  ...
  it('should log via logger.error for status >= 500', async () => {
    const { logger } = await import('@withwiz/toolkit/core/logger/logger');
    const { errorToResponse } = await import('@withwiz/toolkit/next/error/error-handler');
    const { AppError } = await import('@withwiz/toolkit/core/error/app-error');
...
describe('withErrorHandler', () => {
  it('should return handler response when it succeeds', async () => {
    const { withErrorHandler } = await import('@withwiz/toolkit/next/error/error-handler');
