 ...
 * Cache Config Tests
 *
 * src/core/cache/cache-config.ts 유닛 테스트
...
import { getENV, isCacheEnabled } from '@withwiz/core/cache/cache-env';
...
describe('Cache Config', () => {
  ...
  describe('getCacheConfig.analytics', () => {
    it('should return true when both cache and analytics are enabled', async () => {
      const { getCacheConfig } = await import('@withwiz/core/cache/cache-config');
  ...
  describe('getCacheTTL (deprecated aliases)', () => {
    it('should return settings duration', async () => {
      const { getCacheTTL } = await import('@withwiz/core/cache/cache-config');
