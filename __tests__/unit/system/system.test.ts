...
vi.mock('@withwiz/toolkit/core/utils/error-message-formatter', () => ({
  formatRedisError: vi.fn((msg: string) => `[Redis] ${msg}`),
  formatDatabaseError: vi.fn((msg: string) => `[DB] ${msg}`),
}));

// cache mock
vi.mock('@withwiz/toolkit/core/cache/cache', () => ({
...
describe('Environment Check', () => {
  ...
  describe('checkEnvironmentVariables', () => {
    ...
    it('should include NODE_ENV from common config', async () => {
      ...
      const { initializeCommon } = await import('@withwiz/toolkit/core/config/common');
...
describe('Health Check', () => {
  ...
  describe('checkServiceHealth', () => {
    it('should return warning for Database when no prisma client provided', async () => {
      const { checkServiceHealth } = await import('@withwiz/toolkit/core/system/health-check');
